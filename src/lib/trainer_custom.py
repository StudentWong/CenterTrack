...
class GenericLoss(torch.nn.Module):
  ...
  def _sigmoid_output(self, output):
    ...
    if 'hm_old' in output:
      output['hm_old'] = _sigmoid(output['hm_old'])
    if 'hm_new' in output:
      output['hm_new'] = _sigmoid(output['hm_new'])
    if 'hm_hp' in output:
...
class Trainer_Custom(object):
  ...
  def _get_pre_fts(self, pre_imgs, pre_cts_fix, pre_len):
    ...
    N = pre_imgs.shape[0]
    assert pre_imgs.shape[1] == pre_cts_fix.shape[1] and pre_len.shape[1] == pre_cts_fix.shape[1]
    T = pre_imgs.shape[1]
  ...
  def run_epoch(self, phase, epoch, data_loader):
    ...
    for iter_id, batch in enumerate(data_loader):
      ...
      pr_object_ft = self._get_pre_fts(batch['pre_imgs'], batch['pre_cts_fix'], batch['pre_len'])
